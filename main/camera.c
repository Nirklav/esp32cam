...
static CountdownEvent fb_event;
static camera_fb_t* fb;

void camera_wait(CameraListener* listener)
{
    if (listener->registered == false)
    {
        fb_event.max++;
        listener->registered = true;
    }

    countdown_event_wait_max(&fb_event);
    listener->fb = fb;
}
...
{
    listener->fb = NULL;
    countdown_event_decrement(&fb_event);
}
...
void camera_loop()
{
    countdown_event_create(0, &fb_event);
    ...
    while (true)
    {
        ...
        fb = pic;
        countdown_event_reset(&fb_event);
        countdown_event_wait_zero(&fb_event);
